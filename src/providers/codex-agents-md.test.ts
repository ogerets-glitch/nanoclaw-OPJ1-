...
describe('composeGroupAgentsMd scheduling instructions (ncl tasks reach-in)', () => {
  // Red-on-delete guard for the `scheduling`/`cli` exclusion at the
  // module-fragment loop, mirroring src/claude-md-compose.test.ts's guard —
  // the agent is taught `ncl tasks` iff it has ncl.
  beforeEach(() => {
    if (fs.existsSync(TEST_ROOT)) fs.rmSync(TEST_ROOT, { recursive: true });
    fs.mkdirSync(path.join(TEST_ROOT, 'data'), { recursive: true });
    runMigrations(initTestDb());
  ...
  it('includes the scheduling module at the default cli_scope', () => {
    const g = group('sched');
    createAgentGroup(g);
    ensureContainerConfig(g.id);
    const groupDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agents-md-'));
  ...
  it('excludes the scheduling module (and the cli module) when cli_scope is disabled', () => {
    const g = group('sched-off');
    createAgentGroup(g);
    ensureContainerConfig(g.id);
    updateContainerConfigScalars(g.id, { cli_scope: 'disabled' });
    const groupDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agents-md-'));
    try {
      composeGroupAgentsMd(g, groupDir);
      const doc = fs.readFileSync(path.join(groupDir, 'AGENTS.md'), 'utf-8');
