...
describe('SignalAdapter', () => {
  ...
  describe('outbound files', () => {
    it('sends file attachments via a dedicated send RPC call with attachment paths', async () => {
      const adapter = createAdapter();
      await adapter.setup(createMockSetup());
      ...
      const sendCalls = getRpcCallsForMethod('send');
      // One call for the caption text, one for the attachment.
      expect(sendCalls.length).toBe(2);
      const attachmentCall = sendCalls.find((c) =>
        Array.isArray((c.params as { attachments?: unknown[] })?.attachments),
      );
      expect(attachmentCall).toBeDefined();
      const attachments = (attachmentCall!.params as { attachments: string[] }).attachments;
      expect(attachments.length).toBe(1);
      expect(attachments[0]).toContain('hi.txt');
